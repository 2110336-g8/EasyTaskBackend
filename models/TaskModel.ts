...
TaskSchema.pre('save', function (next) {
    ...
    for (const worker of task.hiredWorkers) {
        if (workerIDs.has(worker.workerID.toString())) {
            const error = new TaskValidationError(`Duplicate workerID '${worker.workerID}' within the same task.`);
            return next(error);
        }
        workerIDs.add(worker.workerID.toString());
        
        if (task.customerID.toString() === worker.workerID.toString()) {
            const error = new TaskValidationError(`customerID '${task.customerID}' cannot be equal to any workerID within the same task.`);
            return next(error);
        }
    }
